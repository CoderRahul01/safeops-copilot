...
router.post('/remediate', verifyAuth, async (req, res) => {
  try {
    ...
    // 🛡️ Sentinel: Prevent information disclosure by returning a generic error
    res.status(500).json({ error: 'Remediation protocol failed', message: 'An internal error occurred.' });
  }
});
