...
const processIntent = async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to process intent', message: 'An internal error occurred while processing intent.' });
  }
};
...
const advanceIntent = async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Failed to advance intent', message: 'An internal error occurred while advancing intent.' });
    }
};
