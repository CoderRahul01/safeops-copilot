...
const processIntent = async (req, res) => {
  try {
    ...
    // 🛡️ Sentinel: Prevent information disclosure by returning a generic error message
    res.status(500).json({ error: 'Failed to process intent', message: 'An internal server error occurred.' });
  }
};
...
const advanceIntent = async (req, res) => {
    try {
        ...
        // 🛡️ Sentinel: Prevent information disclosure by returning a generic error message
        res.status(500).json({ error: 'Failed to advance intent', message: 'An internal server error occurred.' });
    }
};
